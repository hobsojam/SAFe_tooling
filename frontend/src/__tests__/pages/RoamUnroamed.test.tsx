...
const unroamedRisks = [
  makeRisk({ id: 'risk-1', description: 'API gateway dependency', roam_status: 'unroamed', team_id: 'team-1' }),
  makeRisk({ id: 'risk-2', description: 'Third party vendor delay', roam_status: 'unroamed', team_id: null }),
];
const roamedRisk = makeRisk({ id: 'risk-3', description: 'Already resolved', roam_status: 'resolved' });

function setupMocks(overrides: {
  isLoading?: boolean;
  risks?: typeof unroamedRisks;
} = {}) {
  ...
  vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === 'risks') return { data: overrides.risks ?? unroamedRisks, isLoading: false } as unknown as ReturnType<typeof useQuery>;
