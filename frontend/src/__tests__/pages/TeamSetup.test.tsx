...
function setupMocks(teams: TeamStub[] = mockTeams): MutationCapture[] {
  ...
  vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === "teams")
      return { data: teams, isLoading: false } as unknown as ReturnType<typeof useQuery>;
...
describe("TeamSetup", () => {
    ...
    it("shows spinner when teams are loading", () => {
      ...
      vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
        ...
        if (key === "pi")
          return { data: mockPi, isLoading: false } as unknown as ReturnType<typeof useQuery>;
        if (key === "teams")
          return { data: [], isLoading: true } as unknown as ReturnType<typeof useQuery>;
        return { data: undefined, isLoading: false } as unknown as ReturnType<typeof useQuery>;
      });
      render(<TeamSetup />);
      expect(screen.getByText("Loading…")).toBeInTheDocument();
  ...
  describe("add team form", () => {
    ...
    it("calls createMut.mutate with form values on valid submit", async () => {
      ...
      await user.click(screen.getByRole("button", { name: "Add Team" }));
      expect(anyMutateCalled(captured, (arg) => (arg as { name?: string }).name === "Gamma")).toBe(
        true
      );
    });
    ...
    it("creates team with selected topology type", async () => {
      ...
      expect(
        anyMutateCalled(
          captured,
          (arg) => (arg as { topology_type?: string }).topology_type === "platform"
        )
      ).toBe(true);
    });
    ...
    it('selecting "None" topology passes null', async () => {
      ...
      expect(
        anyMutateCalled(
          captured,
          (arg) => (arg as { topology_type?: unknown }).topology_type === null
        )
      ).toBe(true);
    });
