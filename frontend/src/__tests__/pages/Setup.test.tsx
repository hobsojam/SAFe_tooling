...
function setupMocks(
  overrides: { isLoading?: boolean; pi?: typeof mockPI; iterations?: typeof mockIterations } = {}
) {
  ...
  vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === "pi")
      return { data: overrides.pi ?? mockPI, isLoading: false } as unknown as ReturnType<
        typeof useQuery
      >;
    if (key === "iterations")
      return {
...
describe("Setup", () => {
  ...
  it("shows spinner when PI data fails to load (isError)", () => {
    vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
      ...
      if (key === "pi")
        ...
        >;
      if (key === "iterations")
        return { data: mockIterations, isLoading: false } as unknown as ReturnType<typeof useQuery>;
      return { data: undefined, isLoading: false } as unknown as ReturnType<typeof useQuery>;
    });
    render(<Setup />);
  ...
  it("shows error message when PI update mutation fails", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Edit" }));
    act(() => {
      onErrors[0]?.(new Error("Update failed"));
    }); // updatePIMut is 1st
