...
function setupMocks(overrides: {
  ...
  vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === 'stories') return { data: overrides.stories ?? mockStories, isLoading: false } as unknown as ReturnType<typeof useQuery>;
...
describe('ARTSync', () => {
  ...
  it('handles stories with no iteration_id gracefully', () => {
    setupMocks({
      stories: [makeStory({ id: 's-unplanned', team_id: 'team-1', iteration_id: null, status: 'done' })],
    });
    render(<ARTSync />);
