...
describe("Backlog page", () => {
  ...
  it("calls delete mutation when user confirms deletion", async () => {
    ...
    const { mutate } = setupQueryMocks(({ queryKey }) => {
      const key = queryKey[0] as string;
      if (key === "pi") return mockPI;
  ...
  it("pre-populates acceptance_criteria and nfr in the edit modal", async () => {
    ...
    const editButtons = screen.getAllByRole("button", { name: "Edit" });
    await user.click(editButtons[0]);
    expect(screen.getByRole("textbox", { name: /acceptance criteria/i })).toHaveValue(
      "Given X When Y Then Z"
    );
    expect(screen.getByRole("textbox", { name: /non-functional requirements/i })).toHaveValue(
      "Handle 1000 RPS"
    );
  });
