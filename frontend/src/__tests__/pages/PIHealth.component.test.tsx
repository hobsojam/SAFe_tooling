...
describe('PIHealth page', () => {
  ...
  it('shows predictability % when objectives are scored', () => {
    setup({
      objectives: [
        makePIObjective({ planned_business_value: 10, actual_business_value: 8, is_stretch: false }),
      ],
    });
    render(<PIHealth />);
    // 8/10 = 80%
  ...
  it('applies amber class for predictability between 60 and 79%', () => {
    setup({
      objectives: [
        makePIObjective({ planned_business_value: 10, actual_business_value: 7, is_stretch: false }),
      ],
    });
    render(<PIHealth />);
    // 7/10 = 70%
  ...
  it('applies red class for predictability < 60%', () => {
    setup({
      objectives: [
        makePIObjective({ planned_business_value: 10, actual_business_value: 5, is_stretch: false }),
      ],
    });
    render(<PIHealth />);
    // 5/10 = 50%
  ...
  it('shows available capacity when a plan exists for a non-IP iteration', () => {
    const plan = makeCapacityPlan({ team_id: 'team-1', iteration_id: 'iter-1', available_capacity: 40 });
  ...
  it('excludes IP iteration from capacity aggregation', () => {
    ...
    const plan = makeCapacityPlan({ team_id: 'team-1', iteration_id: 'iter-ip', available_capacity: 40 });
