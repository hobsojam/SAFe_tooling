...
function setupMocks(overrides: {
  ...
  vi.mocked(useQuery).mockImplementation((opts: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === 'improvement-actions') return { data: overrides.actions ?? [], isLoading: false } as unknown as ReturnType<typeof useQuery>;
...
describe('InspectAdapt', () => {
  ...
  it('renders fallback team name when an objective has no team', () => {
    setupMocks({
      objectives: [
        makePIObjective({ id: 'obj-no-team', team_id: null as any, pi_id: 'pi-1' }),
      ],
    });
    render(<InspectAdapt />);
  ...
  it('shows improvement action mutation errors in the modal', async () => {
    ...
    render(<InspectAdapt />);
    await user.click(screen.getByRole('button', { name: '+ New Action' }));
    act(() => { onErrors[0]?.(new Error('Create failed')); });
  ...
  it('shows delete errors for improvement actions', async () => {
    ...
    render(<InspectAdapt />);
    await user.click(screen.getAllByRole('button', { name: 'Delete' })[0]);
    act(() => { onErrors[2]?.(new Error('Delete failed')); });
