...
function setupMocks(overrides: { pis?: typeof mockPIs; arts?: typeof mockARTs } = {}) {
  ...
  vi.mocked(useQuery).mockImplementation(({ queryKey }: Parameters<typeof useQuery>[0]) => {
    ...
    if (key === "pis")
      return { data: overrides.pis ?? mockPIs } as unknown as ReturnType<typeof useQuery>;
    if (key === "arts")
      return { data: overrides.arts ?? mockARTs } as unknown as ReturnType<typeof useQuery>;
