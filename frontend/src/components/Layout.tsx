...
export function Layout() {
  ...
  function handlePISubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!piForm.name.trim()) {
      setPiError("Name is required.");
      return;
    }
    if (!piForm.art_id) {
      setPiError("ART is required.");
      return;
    }
    if (!piForm.start_date || !piForm.end_date) {
      setPiError("Start and end dates are required.");
      return;
    }
        ...
        >
          ...
          <div className="px-3 pb-4">
            <div className="mb-1 flex items-center justify-between gap-2">
              <label htmlFor="pi-select" className="block text-xs text-slate-400">
                Program Increment
              </label>
              <button
                type="button"
              ...
              <div className="mt-3 space-y-1">
                {NAV_GROUPS.map((group) => {
                  const groupIsActive = group.items.some(
                    ({ to }) => location.pathname === `/pi/${piId}/${to}`
                  );
                  return (
                    <details key={group.heading} open={groupIsActive}>
                      <summary className={navSectionClass(groupIsActive)}>{group.heading}</summary>
          ...
          <div className="mt-auto border-t border-slate-700 px-2 pt-2">
            <details open={setupIsActive}>
              <summary className={navSectionClass(setupIsActive)}>Setup</summary>
              <div className="mt-1 space-y-0.5">
                {piId &&
                  PI_SETUP_ITEMS.map(({ to, label, primary }) => (
                    <NavLink
                      key={to}
                      to={`/pi/${piId}/${to}`}
                      onClick={closeSidebar}
